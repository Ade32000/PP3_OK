...
function checkContent(tx)
{
    ...
    tx.executeSql('SELECT * FROM categories;', [], function(tx, result){
        ...
        //     $('.dropdown-menu').append('<a class="dropdown-item" href="#">'+resultat[i].category_name+'</a>');
        // }
        //$("#display").append("<img src='" + result.rows[0].data + "' alt='logo'></img>");
        // tx.executeSql('SELECT * FROM pictures;', [], function(tx,result){
        //     var pic = result.rows;
        //     console.log(result.rows);
        //     for(var y=0; y<pic.length; y++){
        //         $('#display').append('<img accept=".jpg, .jpeg, .png, .svg, .JPG, .JPEG, .PNG, .SVG"  src="'+pic[y].picture_url+'"alt="'+pic[y].picture_name+'"/>');
        //     }
        // })
    });
}
...
function populateDB(tx)
{
    ...
    // tx.executeSql('UPDATE pictures SET picture_url="images/asperges 1.JPG" WHERE picture_id=4');
